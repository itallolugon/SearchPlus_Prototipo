...
def _rotas():
    return {
        "SELECT id, folder_id, nome, caminho": {"fetchall": []},
...
class TestFiltroDeFavoritos:
    def test_liga_o_filtro(self, client_logado, db_roteado, app_module):
        ...
        with (
            ...
            client_logado.post(
                "/api/search", json={"query": "praia", "avancado": {"so_favoritos": True}}
            )

        assert "favorito = 1" in _sql_principal(conexao)

    def test_sem_o_filtro_a_busca_e_a_de_sempre(self, client_logado, db_roteado, app_module):
        conexao = db_roteado(_rotas())
        with (
            mock.patch.object(app_module, "SBERT_OK", True),
            mock.patch.object(app_module, "_SBERT", _sbert_falso()),
    ...
    def test_valor_falso_nao_liga_o_filtro(self, client_logado, db_roteado, app_module):
        """
        ...
        with (
            ...
            client_logado.post(
                "/api/search", json={"query": "praia", "avancado": {"so_favoritos": False}}
            )

        assert "favorito = 1" not in _sql_principal(conexao)

    def test_compoe_com_os_outros_filtros(self, client_logado, db_roteado, app_module):
        """
