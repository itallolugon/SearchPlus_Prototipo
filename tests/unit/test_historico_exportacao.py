...
def _registro(**extra):
    base = {"id": 1, "collection_id": 3, "colecao": "Viagem",
            "pasta": "D:/Fotos/Viagem", "total": 12, "copiados": 9,
            "falhas": _falhas(("a.jpg", "sem_permissao"),
                              ("b.jpg", "nao_encontrado")),
            "estado": "concluido", "quando": None}
    base.update(extra)
    return base
...
class TestListagem:
    ...
    def test_diz_se_a_pasta_ainda_existe(self, client_logado, db_roteado, tmp_path):
        ...
        """
        db_roteado({"FROM exportacoes": {"fetchall": [
    ...
    def test_aceita_falhas_como_texto(self, client_logado, db_roteado):
        ...
        db_roteado({"FROM exportacoes": {"fetchall": [
            _registro(falhas=json.dumps(_falhas(("x.jpg", "erro_leitura"))))]}})
...
class TestRegistro:
    def test_falha_ao_registrar_nao_derruba_o_worker(self, app_module, db_roteado):
        ...
        app_module._registrar_exportacao({
            "user_id": 1, "collection_id": 1, "colecao": "V", "pasta": "D:/x",
            "total": 1, "copiados": 1, "falhas": [], "estado": "concluido"})
...
class TestRepetir:
    ...
    def test_repete_so_o_que_pode_dar_certo(self, client_logado, db_roteado, tmp_path):
        ...
        conexao = db_roteado(self._rotas(
            ...
            arquivos=[{"nome": "a.jpg", "caminho": "C:/Fotos/a.jpg",
                       "tipo": "jpg", "data_adicionado": None}]))
    ...
    def test_so_sumidos_nao_da_o_que_repetir(self, client_logado, db_roteado, tmp_path):
        db_roteado(self._rotas(reg=_registro(
            pasta=str(tmp_path), falhas=_falhas(("b.jpg", "nao_encontrado")))))
    ...
    def test_arquivos_fora_da_colecao_e_recusado(self, client_logado, db_roteado,
                                                  tmp_path):
...
class TestLimparSumidos:
    ...
    def test_tira_da_colecao_o_que_sumiu(self, client_logado, db_roteado, tmp_path):
        conexao = db_roteado(self._rotas(candidatos=[
            {"id": 7, "nome": "b.jpg", "caminho": str(tmp_path / "sumida.jpg"),
             "adicionado_em": None}]))
    ...
    def test_arquivo_que_voltou_nao_e_removido(self, client_logado, db_roteado,
                                                tmp_path):
        """
        ...
        conexao = db_roteado(self._rotas(candidatos=[
            {"id": 7, "nome": "b.jpg", "caminho": str(voltou),
             "adicionado_em": None}]))
        corpo = client_logado.post("/api/exportacoes/1/limpar_sumidos").get_json()
    ...
    def test_exportacao_alheia_da_404(self, client_logado, db_roteado):
        db_roteado(self._rotas(reg=None))
        assert client_logado.post(
            "/api/exportacoes/999/limpar_sumidos").status_code == 404
