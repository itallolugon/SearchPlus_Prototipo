        ...
        db_roteado(
            {
                "SELECT id, nome, criado_em": {
                    "fetchone": {
                        ...
                        "nome": "Viagem",
                        "criado_em": None,
                        "pasta_vinculada": r"D:\Viagem",
                        "modo_sync": "auto",
                    }
                },
                "SELECT file_id, adicionado_em": {
                    "fetchall": [
                        {"file_id": 11, "adicionado_em": None},
                        {"file_id": 12, "adicionado_em": None},
                    ]
                },
                "SELECT caminho, recebe": {
                    "fetchall": [{"caminho": r"D:\Viagem", "recebe": True, "criado_em": None}]
                },
            }
        )
        conn = app_module.get_db()
...
class TestRestaurar:
    ...
    def test_devolve_as_pastas_geradas(self, app_module, db_roteado):
        ...
        app_module._restaurar_colecao(
            conexao,
            1,
            self._retrato(pastas=[{"caminho": r"D:\Viagem", "recebe": True, "criado_em": None}]),
        )
    ...
    def test_itens_voltam_para_a_colecao(self, app_module, db_roteado):
        ...
        ok, _ = app_module._restaurar_itens(
            conexao, 1, {"collection_id": 7, "arquivos": [{"file_id": 11, "adicionado_em": None}]}
        )
    ...
    def test_itens_sem_colecao_explicam_o_que_fazer(self, app_module, db_roteado):
        ...
        ok, motivo = app_module._restaurar_itens(conexao, 1, {"collection_id": 7, "arquivos": []})
...
class TestEndpointRestaurar:
    def _rotas(self, tipo="colecao", conteudo=None):
        ...
        return {
            "SELECT id, tipo, rotulo, conteudo FROM lixeira": {
                "fetchone": {"id": 5, "tipo": tipo, "rotulo": "Viagem", "conteudo": conteudo}
            },
            "SELECT id FROM collections WHERE user_id": {"fetchone": None},
            "SELECT id FROM collections WHERE id": {"fetchone": {"id": 7}},
...
class TestListarEDescartar:
    def test_lista_o_que_esta_na_lixeira(self, client_logado, db_roteado):
        db_roteado(
            {
                "SELECT id, tipo, rotulo, excluido_em": {
                    "fetchall": [
                        {
                            "id": 5,
                            "tipo": "colecao",
